...
CLASS lhc_attendance IMPLEMENTATION.
  ...
  METHOD fill_initial_date_time.
      READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY attendance FIELDS ( EntryDate TimeIn TimeOut )
    ...
    IF lv_assigned = abap_true.
      MODIFY ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY attendance
  ...
  METHOD check_entry_date.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY attendance
    FIELDS ( ContractorKey EntryDate )
    WITH CORRESPONDING #( keys )
    RESULT DATA(lt_attend_dates).
    LOOP AT lt_attend_dates ASSIGNING FIELD-SYMBOL(<ls_attend_dates>).
       READ ENTITIES of zdemo_i_contractor in local mode entity contractor
