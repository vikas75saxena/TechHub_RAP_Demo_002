...
CLASS lhc_contractor IMPLEMENTATION.
 ...
 METHOD get_features.
 READ ENTITIES OF ZDEMO_I_CONTRACTOR IN LOCAL MODE
 ENTITY contractor
...
ENDMETHOD.
  ...
  METHOD check_dates.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( StartDate EndDate )
    WITH CORRESPONDING #( keys )
    ...
    LOOP AT lt_contr_dates ASSIGNING FIELD-SYMBOL(<ls_contr_dates>).
      ...
      AND <ls_contr_dates>-EndDate IS NOT INITIAL.
        ...
        APPEND VALUE #(  %tky = <ls_contr_dates>-%tky
                         %msg = new_message( id = 'ZDEMO_CONTR'
  ...
  METHOD fill_contractor_id.
    DATA: lv_contractorid TYPE zcontractor_id,
          onr_ref         TYPE REF TO cx_number_ranges,
          onrobj_ref      TYPE REF TO cx_nr_object_not_found.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( ContractorId )
    WITH CORRESPONDING #(  keys )
    ...
    IF lv_number_assigned = abap_true.
      ...
      MODIFY ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor
  ...
  METHOD fill_end_date.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( Status EndDate )
    WITH CORRESPONDING #( keys )
  ...
  METHOD fill_initial_status.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( Status )
    WITH CORRESPONDING #(  keys )
  ...
  METHOD fill_start_date.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( StartDate )
    WITH CORRESPONDING #(  keys )
  ...
  METHOD fill_initial_country.
    READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( Country )
    WITH CORRESPONDING #(  keys )
...
*                                    number = '004' ) ) TO reported-contractor.
 ...
 endloop.
 MODIFY ENTITIES OF ZDEMO_I_CONTRACTOR IN LOCAL MODE
 ENTITY contractor
...
* result = 1.
  ...
  METHOD check_names.
      READ ENTITIES OF zdemo_i_contractor IN LOCAL MODE ENTITY contractor FIELDS ( FirstName LastName )
    WITH CORRESPONDING #( keys )
